...
class BiFPN3D(nn.Module):
    def __init__(self, num_channels, conv_channels, first_time=False, epsilon=1e-4, onnx_export=False, attention=True):
        ...
        self.conv3_up = SeparableConvBlock3D(num_channels//2, onnx_export=onnx_export)
        ...
        # 在BiFPN3D类的__init__中添加一个新的1x1卷积层用于p4_td的通道调整
        self.p4_td_channel_reduce = nn.Conv3d(num_channels, num_channels//2, kernel_size=1)

        # 添加p3_out的通道扩展层
        self.p3_out_channel_expand = nn.Conv3d(num_channels//2, num_channels, kernel_size=1)
...
if __name__ == '__main__':
    ...
    # 创建五层特征图，匹配编码器输出
    p3_in = torch.rand(1, 64, 10, 112, 112)
    p4_in = torch.rand(1, 128, 10, 56, 56)
    p5_in = torch.rand(1, 128, 10, 28, 28)
    p6_in = torch.rand(1, 128, 10, 14, 14)
    p7_in = torch.rand(1, 128, 5, 7, 7)
