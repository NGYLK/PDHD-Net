...
from nndet.arch.decoder.BiFPN import BiFPN3D  # 导入 BiFPN 模块
...
class BaseUFPN(nn.Module):
    ...
    def compute_output_channels(self) -> List[int]:
        ...
        if self.decoder_levels is not None:
            output_levels = list(range(self.num_level))
            # filter for levels above decoder levels
            output_levels = [ol for ol in output_levels if ol < min(self.decoder_levels)]
            assert max(output_levels) < min(self.decoder_levels), "Can not decrease channels below decoder level"
            for ol in output_levels[::-1]:
