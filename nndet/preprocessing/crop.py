...
def crop_to_nonzero(data, seg=None, nonzero_label=-1):
    ...
    """
    logger.info(f"Initial shapes - data: {data.shape}, seg: {seg.shape if seg is not None else None}")
    
    nonzero_mask = create_nonzero_mask(data)
    logger.info(f"Nonzero mask shape: {nonzero_mask.shape}")
    ...
    logger.info(f"After crop shapes - data: {data.shape}, seg: {seg.shape}, nonzero_mask: {nonzero_mask.shape}")
