...
# What is PDHD-Net?
PDHD-Net is a Prostate Dual-branch Hybrid-domain prediction network built on nnDetection, targeting fine-grained Gleason Grade Group (GGG 1–5) stratification from multi-parametric MRI (mp-MRI). We introduce a frequency-band-shunting feature enhancement: high-frequency cues amplify subtle lesion–tissue textures while low-frequency cues emphasize global structure; combining convolutional networks for local detail with self-attention for global semantics improves lesion localization and GGG prediction. On multi-center data (including ProstateX-2 and private cohorts), PDHD-Net achieves 96.7% sensitivity for high-risk lesions (GGG=5) and 88.4% for clinically significant lesions at 1 false positive per patient; for the common binary task (clinically significant GGG≥2 vs GGG=1), it attains an AUC of 0.82. Code: https://github.com/NGYLK/PDHD-Net. Built on nnDetection (Baumgartner et al., MICCAI 2021).
...
<details close>
<summary>PDHD-Net Functional Details</summary>
<br>
    <div align="center">
        <img src=docs/source/9-13_1.pdf width="600px">
    </div>
</details>

# Experiments & Data
The internal cohort comprised 500 mp-MRI cases of histologically confirmed prostate cancer from Shanghai Tertiary Hospital (single site), acquired on 3T Siemens Magnetom Prisma scanners. Sequences included T2-weighted (T2W: TR/TE=4000/30 ms, slice thickness=3 mm), apparent diffusion coefficient (ADC: derived from DWI), diffusion-weighted imaging at b=1000 s/mm² (DWI1000: TR/TE=5000/60 ms, slice thickness=3 mm) and b=3000 s/mm² (DWI3000: TR/TE=5000/70 ms, slice thickness=3 mm), and dynamic contrast-enhanced (DCE) imaging (pre- and initial post-contrast phases). No b-values were harmonized via ComBat due to single-site homogeneity.\par
Lesions were manually delineated voxel-wise by two senior genitourinary radiologists (10+ years experience) using consensus resolution for discrepancies (inter-reader Kappa=0.85, 95\% CI: 0.78-0.92). The reference standard was Gleason Grade Group (GGG) from systematic/targeted biopsy (n=450) or radical prostatectomy (n=50), with median imaging-biopsy interval of 14 days (IQR: 7-21). For multi-focal cases (n=120, 24\%), per-lesion GGG assignment reconciled biopsy-miss via nearest-lesion mapping and pathologist adjudication; inconsistencies (5\%) resolved by consensus. GGG distribution: GGG1 (15\%), GGG2 (25\%), GGG3 (30\%), GGG4 (20\%), GGG5 (10\%).\par
The external test set (n=60) from the public ProstateX-2 challenge was held out entirely, acquired on mixed 3T scanners (Siemens/GE; heterogeneous parameters: T2W TR/TE=3000-5000/20-40 ms, DWI b=0-1000 s/mm², slice=3-4 mm). Preprocessing mirrored the internal pipeline, excluding ComBat harmonization. Approximately 30\% of cases exhibited multi-focality, with 2-3 lesions per such case. GGG distribution was balanced across levels 1-5.
...
## Results
[PDHD-001 v0.1](./docs/results/combined_froc_analysis.pdf)
</div>
...
## Using PDHN-Net
The following paragrah provides an high level overview of the functionality of PDHN-Net and which commands are available.
...
Before training the networks, PDHN-Net needs to preprocess and analyze the data.
